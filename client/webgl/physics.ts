...
Object.defineProperty(window, "rapier", {
  get() {
    return mustExist(rapierInstance);
  },
  configurable: false,
  enumerable: false,
});
...
export class PhysicsSystem extends System {
  ...
  protected updateRigidBodyPosition(
    transform: Transform,
  ...
  protected updateRigidBodyPositions() {
    for (const [
      ...
      rigidBodyComponent,
      transform,
    ] of this.world.componentIterator(RigidBodyComponent, Transform)) {
      this.updateRigidBodyPosition(transform, rigidBodyComponent.impl!);
    }
  }

  protected updateEntityTransform(
    transform: Transform,
    body: Rapier.RigidBody,
  ) {
