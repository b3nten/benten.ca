...
export class ComputerMovementSystem extends System {
  ...
  update(delta: number) {
    for (const [, rbody, transform, bmvsys] of this.world.componentIterator(
      RigidBodyComponent,
      Transform,
      ComputerMovementComponent,
    )) {
      const rb = rbody.impl;
      if (!rb) continue;
  ...
  _onScroll() {
    const ballCollider = this.world
      .componentIterator(ResizeBallComponent, ColliderComponent)
      .next();
    if (!ballCollider.done) {
      const rb = ballCollider.value[2].impl;
      if (rb) {
        const size = remapRange(
          window.scrollY,
          0,
...
export function createComputerPrefabFactory() {
  ...
  const monitorCollider = rapier.ColliderDesc.convexHull(
    <Float32Array>(
      (<Three.Mesh>monitor.children[0]).geometry.attributes.position.array
    ),
  )!
    .setMass(6)
    .setFriction(0.5);
  ...
  const terminalCollider = rapier.ColliderDesc.convexHull(
    <Float32Array>(<Three.Mesh>terminal).geometry.attributes.position.array,
  )!
    .setMass(6)
    .setFriction(0.5);
  ...
  return (args: { scale: number }) => {
    return createPrefab((world) => {
      ...
      const mesh = rand > 0.25 ? monitor : terminal;
