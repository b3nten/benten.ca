...
export class Window extends WebComponent("ui-window")
{
  ...
  `;
	...
	override onMount()
	{
		const children = Array.from(this.parentElement!.childNodes).filter((x) => x instanceof Window);
		let highest = -1;
	...
	override onMounted()
	{
		let existingWindows = Array.from(this.parentElement!.childNodes).filter((x) => x instanceof Window);
