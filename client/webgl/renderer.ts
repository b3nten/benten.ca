...
export class CustomRenderPipeline implements IRenderPipeline {
  ...
  bloom = new TransparentBloom(new Three.Vector2(512, 512), 1.5, 0.4, 0.9);
  ...
  render = (
    ...
    this.saoPass?.render(
      renderer,
      this.swapChain.writable,
      this.swapChain.readable,
      delta,
      false,
    );
    ...
    // pixel effect
    this.pixelPass.prerender(
      renderer,
      this.swapChain.readable.texture,
      this.prepass.depthTexture,
      this.prepass.normalTexture,
    );
  ...
  preparePasses(scene: Three.Scene, camera: Three.Camera, viewport: Viewport) {
    ...
    this.backgroundPass.shader.uniforms["u_MousePos"].value.set(
      Input.mouseX,
      Input.mouseY,
    );
    this.backgroundPass.shader.uniforms["u_MouseVelocity"].value.set(
      Input.mouseDeltaX,
      Input.mouseDeltaY,
    );
  }
  ...
  onScroll() {
    ...
    const doubledScrollAmount = remapRange(
      window.scrollY,
      0,
    ...
    this.uberPass.shader.uniforms["u_Brightness"].value = remapRange(
      doubledScrollAmount,
      1,
      0,
      -2,
